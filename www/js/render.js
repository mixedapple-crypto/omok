 ...
 * 판의 최대 크기.
 *
 * 720인 이유: 아이패드 세로는 폭이 820px 안팎이라 560으로 자르면 화면의 3분의 1이
 * 빈 채로 남고 판이 작아 보인다. 720이면 한 칸이 약 47px로, 아이 손가락에 넉넉하다
 * (폰에서는 어차피 화면 폭이 먼저 걸려 이 값에 닿지 않는다).
 * 상한 자체를 없애지 않는 이유는 큰 화면에서 판이 팔을 뻗어야 닿을 만큼 커지면
 * 오히려 두기 불편해서다.
 */
const MAX_BOARD_PX = 720;
